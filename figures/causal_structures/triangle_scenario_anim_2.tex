\begin{tikzpicture}[scale=1]
    ...
    \begin{scope}[every node/.style=observed, onslide=<1->{fade}]
        \node[onslide=<1>{unfade}] (C) at (-2, 0) {$C$};
        \node (B) at (2, 0) {$B$};
        \node[onslide=<1>{unfade}] (A) at (0, {2*sqrt(3)}) {$A$};
    \end{scope}
    \begin{scope}[every node/.style=latent, onslide=<1->{fade}]
        \node[onslide=<1>{unfade}] (X) at (-1, {sqrt(3)}) {$X$};
        \node[onslide=<1>{unfade}] (Y) at (1, {sqrt(3)}) {$Y$};
        \node[onslide=<1>{unfade}] (Z) at (0, 0) {$Z$};
    \end{scope}
